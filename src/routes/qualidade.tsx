...
function AdminQualidade() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: erros, error: erroErros }, { data: logs, error: erroLogs }] =
        await Promise.all([
      ...
      setErroCarga(Boolean(erroErros || erroLogs));
      setErrosApp(erros ?? []);
      setLogsFuncao(logs ?? []);
      setCarregando(false);
    })();
  ...
  return (
    <>
      ...
      {erroCarga && (
        <div className="mb-6 rounded-xl border border-[var(--danger)]/25 bg-[var(--danger-soft)] p-4">
          <p className="font-sans text-[13px] text-[var(--danger)]">
            Não consegui carregar os dados de qualidade. Os números abaixo podem estar incompletos.
          </p>
        </div>
      )}
      ...
      <div className={`${CARD_CLASS} mb-6 p-6`}>
        <p className="mb-5 font-accent text-[11px] font-bold uppercase tracking-[2px] text-[var(--muted)]">
